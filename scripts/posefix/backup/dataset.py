...
class Dataset(object):
    ...
    def load_train_data(self):
        ...
        train_data = []
        print("Load train data from: ", self.train_annot_path)
        for aid in coco.anns.keys():

            ann = coco.anns[aid]
            imgname = 'train2017/' + coco.imgs[ann['image_id']]['file_name']
            joints = ann['keypoints']
            """if ann['image_id'] == 139081313:
                print(ann)
                ...
                if (np.sum(joints[2::3]) == 0):
                    print("(np.sum(joints[2::3]) == 0)")
                if (ann['num_keypoints'] == 0):
                    print("(ann['num_keypoints'] == 0):")"""
        ...
        #with open(os.path.join('/home/althausc/master_thesis_impl/PoseFix_RELEASE/inputs','test.json'), 'w') as f:
    ...
    def evaluation(self, result, gt, result_dir, db_set):
        result_path = osp.join(result_dir, 'resultfinal.json')
