...
def plot_images(
    ...
    if labels is None:
        ...
        labels.insert(0,'')
    scores.insert(0,'Query Image')
    html = _create_imgs_list_html(
        images, labels, scores, max_images, img_width, force_b64=force_b64)
...
def _create_img_html(image, width, label, score, force_b64=False):
    html = (
    ...
    html += '<span style="font-size: 12px">%s</span>' %str(score)

    return html + '</div>'


def _create_imgs_list_html(images, labels, scores, max_images, img_width, force_b64=False):
    print(scores)
    html = ''.join([
        _create_img_html(x, img_width, label=y, score=z, force_b64=force_b64)
        for x, y, z in zip(images[:max_images], labels[:max_images], scores[:max_images])
    ])
    return html
