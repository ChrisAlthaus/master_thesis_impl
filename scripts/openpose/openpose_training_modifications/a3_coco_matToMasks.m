...
% for mode = 7 % For face70
    ...
    mkdir(imageSegmentationFolder) %modified

    numberImagesWithPeople = length(matAnnotations);
    disp(['Number of images with people= ',num2str(numberImagesWithPeople)])
    ...
    for imageIndex = 1:numberImagesWithPeople
        ...
        % Paths
        if mode == 1    %modified to account for composed image names (contentid_styleid.jpg)
            imageId = num2str(matAnnotations(imageIndex).image_id);
            %Style image has always 6 digits
            contentImNumber = extractBetween(imageId, 1, length(imageId)-6);
            styleImNumber = extractBetween(imageId, length(imageId)-5, length(imageId));

            contentImFilename = sprintf('%012d', str2num(contentImNumber{1,1}));
            styleImFilename = styleImNumber{1,1};

            imageFilename = [contentImFilename, '_', styleImFilename, '.jpg'];
            maskMissPath = [imageMaskFolder, imageFilename];
            segmentationPath = [imageSegmentationFolder, imageFilename];
            imagePath = [dataType, '/', imageFilename];
