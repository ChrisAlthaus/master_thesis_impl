# Path: /home/althausc/master_thesis_impl/Scene-Graph-Benchmark.pytorch/maskrcnn_benchmark/data/datasets/evaluation/vg/vg_eval.py
import logging
import os
import torch
import numpy as np
...
def do_vg_evaluation(
    ...
    zeroshot_triplet = torch.load("/home/althausc/master_thesis_impl/Scene-Graph-Benchmark.pytorch/maskrcnn_benchmark/data/datasets/evaluation/vg/zeroshot_triplet.pytorch", map_location=torch.device("cpu")).long().numpy()
    ...
    if "relations" in iou_types:
        ...
        #modified: get all evaluation metrics for logging
        print("Evaluator: ",evaluator)
        print("Evaluator previous: ",float(np.mean(result_dict[mode + '_recall'][100])))
        return evaluator, float(np.mean(result_dict[mode + '_recall'][100]))
