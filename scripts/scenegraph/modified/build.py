#Path: /home/althausc/master_thesis_impl/Scene-Graph-Benchmark.pytorch/maskrcnn_benchmark/data/build.py
...
def get_dataset_statistics(cfg):
    ...
    for dataset_name in dataset_names:
        print("Dataset name stats: ",dataset_name)
...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True):
    ...
    for dataset_name in dataset_list:
        print("Get dataset with name: ", dataset_name)
        ...
        #modified
        print("Number of images in loaded dataset {} = {}".format(dataset_name, len(dataset)))
        #modified end
...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0):
    assert mode in {'train', 'val', 'val2', 'test'}
    ...
    if is_train:
        ...
        assert (
            ...
            images_per_batch, num_gpus)
        images_per_gpu = images_per_batch // num_gpus
        shuffle = False if not is_distributed else True #False
        num_iters = None
        start_iter = 0
